...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Feed, c.Item, c.Read, c.Subscription, c.Token, c.User,
	} {
		n.Use(hooks...)
	}
}
...
// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *ReadClient) MapCreateBulk(slice any, setFunc func(*ReadCreate, int)) *ReadCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &ReadCreateBulk{err: fmt.Errorf("calling to ReadClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*ReadCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
type (
	hooks struct {
		Feed, Item, Read, Subscription, Token, User []ent.Hook
	}
	inters struct {
		Feed, Item, Read, Subscription, Token, User []ent.Interceptor
	}
)
